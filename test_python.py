...
class NavigateCategory:
    ...
    def get_base_header_with_token(self):
        ...
        base_header['sessionid'] = self.sessionid
        return base_header

    def set_csrf_token_before_login(self):
        response = self.session.get("{}/api/user/v2/account/login_session/".format(
                                        self.host))  # https://learn.sdaia.academy/csrf/api/v1/token
        if response.status_code == 200 and response.cookies['csrftoken']:
