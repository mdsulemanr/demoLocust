...
class UserLogin(SequentialTaskSet):
    ...
    def get_base_header(self):
        base_header = {
            ...
            # 'Sec-Fetch-User': '?1',
            # # 'X-CSRFToken': token,
            'Origin': 'https://academy.mckinseyaccelerate.com',
            'Referer': 'https://academy.mckinseyaccelerate.com/learn',
    ...
    #     base_header['_csrf'] = self.csrftoken
    #     return base_header
    ...
    def on_start(self):
        ...
        data = {
  "username": "Test1-AVC-305@mckinsey.com",
  ...
  "client_timezone_textual": "Asia/Karachi"
        ...
        with self.client.post("/manage/v1/user/login?lang=en", data, headers=self.get_base_header(),
                              catch_response=True) as response:
            if response.status_code == 200 and "access_token" in response.text:
                # print(response.text)
                # print(response.cookies)
                self.csrftoken = response.cookies['_csrf']
                print("login successful with " + "Test1-AVC-305@mckinsey.com")
                # print(response.cookies)
    ...
    def navigate_to_notifications(self):
        with self.client.get("/lms/index.php?r=inbox/inbox/all",
                             catch_response=True) as response:
            if response.status_code == 200:
                # print(response.text)
                if "Notifications" in response.text:
                    response.success()
                else:
                    response.failure("Notifications string not found" + str(response.status_code))
...
class MyUser(HttpUser):
    ...
    host = 'https://academy.mckinseyaccelerate.com'
    tasks = [UserLogin]
